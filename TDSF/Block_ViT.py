...
class Block_ViT(nn.Module):
    def __init__(self, config, channel):
        ...
        self.attn_norm1 = nn.LayerNorm(channel[0], eps=1e-6)
        self.attn_norm2 = nn.LayerNorm(channel[1], eps=1e-6)
        self.attn_norm3 = nn.LayerNorm(channel[2], eps=1e-6)

        self.attn_norm = nn.LayerNorm(sum(channel), eps=1e-6)
        self.channel_attn = Attention_org(config, channel)

        self.ffn_norm1 = nn.LayerNorm(channel[0], eps=1e-6)
        self.ffn_norm2 = nn.LayerNorm(channel[1], eps=1e-6)
        self.ffn_norm3 = nn.LayerNorm(channel[2], eps=1e-6)

        self.ffn1 = Mlp(channel[0], channel[0]*expand_ratio)
        self.ffn2 = Mlp(channel[1], channel[1]*expand_ratio)
        self.ffn3 = Mlp(channel[2], channel[2]*expand_ratio)
